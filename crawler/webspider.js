...
function  startRequest(x) {
    ...
    http.get(x, function (res) {
        ...
        res.on('end', function () {
            ...
            news_title = $('div.article-title a').text().trim();
...
function  savedImg($,news_title) {
    $('.article-content img').each(function (index, item) {
        ...
        //采用request 模块，向服务器发送一次请求，获取图片资源
        request.head(img_src,function (err,res,body) {
            if(err) console.log(err)
        });
        request(img_src).pipe(fs.createWriteStream('./image/'+news_title+'---'+img_filename));  //通过流的方式，把图片写到/image目录下，并用新闻的标题和图片的标题作为图片的名称。
